 ...
 * otherwise assert.
 *
 * Canonical location: this hook/Modal/ConfirmDialog were promoted here from
 * `components/demo/cafe/**` (Cafe manager-parity design-kit work) so any
 * module/package can reuse them, not just Cafe. `components/demo/cafe/**`
 * now holds thin re-export shims only — see that directory's own
 * `useRestoreFocusOnClose.test.ts` for the shim-correctness check.
 */
