...
| DEBT-038 | `apps/web` `test` script is an explicit file list, and it has already drifted. On 2026-09-19: the script lists 130 paths (129 under `src` plus `scripts/verify-preview-server-actions.test.ts`) and 130 test files exist under `src`, so the counts match by coincidence. **4 real tests are never run** (`inventory/inventory-i18n.test.ts`, `recipes/recipes-i18n.test.ts`, `lib/preview/actions/settings-actions.test.ts`, `lib/workforce/estimated-earnings.test.ts`; run by hand they pass, 13/13) and **3 listed paths no longer exist** (`dashboard/inventory/inventory-i18n.test.ts`, `dashboard/workforce/recipes/recipes-i18n.test.ts`, `lib/preview/actions/recipe-translation-actions.test.ts`, stale since the routes moved). On Node v24.15.0 `node --test` prints "Could not find" for a missing path and still exits 0, which is why the suite passes; CI's Node version was not tested. Fix: correct the list and replace it with a glob, or add a guard test that compares the list to disk. | A | `apps/web/package.json` | T-ACCEPT | Lead | OPEN |
