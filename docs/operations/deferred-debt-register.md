...
| ID | Item | Class | Source | Trigger | Owner | Status |
|---|---|---|---|---|---|---|
| DEBT-050 | **F-06:** `issues.issues.business_date` defaults to `current_date` (UTC) and `api.issues_create` does not set it: an issue reported 00:00-09:00 JST got yesterday's 営業日. **Fixed and applied to Cloud DEV 2026-09-22 (migration `0122`, verified live: an issue created at 03:04 JST got 2026-09-22).** Residual: issues created BEFORE `0122` keep their UTC-derived dates (the 0118 update guard blocks a correction; a backfill would disable the guard inside a migration and set `business_date` from `created_at` in the location timezone). A Founder decision, only matters for old QA rows. | C | Integrated Acceptance F-06; Mission 10.5 | T-DECISION | Founder | OPEN (history only) |
...
| DEBT-060 | NOT TESTED live in the 2026-09-21 acceptance (each needs a mutation or state that was out of reach or would have added QA residue): correction requests (Staff submit, Manager decide), Staff invitations and LINE linking, Settings edits (headcount, shift types), Transport, Work report, Staff-side exchange accept and cancel-own-request; error, degraded and module-OFF states; performance of Issues, Manager Schedule week navigation and any run under network/CPU throttling; the "概算人件費" sum with real worked hours for two different employees (Mission 10.5: rates persisted live, the sum is proven by unit tests only, because the single Staff account cannot produce attendance for a second employee and today's own shift cell does not open by design); the Inventory-OFF gate of `0122` live (needs toggling a tenant module on Cloud, a data write: pgTAP only, see DEBT-049). | C | Integrated Acceptance report §13 rows 3b, 6b, 9b | T-ACCEPT | Lead | OPEN |
| DEBT-061 | **Cloud DEV: VERIFIED PASS 2026-09-21** (project ref given by the Founder, read-only probe with the public publishable key from a browser on the project's own origin): the Data API exposes only `public`, `graphql_public`, `api`; `inventory`, `workforce`, `core`, `purchases`, `issues`, `operations`, `audit`, `ai` answer `406 PGRST106`; `api.*` answers `42501 permission denied` to the anon key. **Still open: the same probe against the Production project** (`Accept-Profile` per schema on `/rest/v1/<table>?limit=1`; an exposed `inventory` would make `reference_unit_price` readable by Staff, a P1). Repeat on any Data API settings change. | A | DB security review 2026-09-21 | T-PROD | Founder gives the Prod ref and key, Lead checks | OPEN (Prod only) |
...
| DEBT-065 | Supabase notice (2026-09): from 2026-10-30 new tables in the `public` schema get no automatic Data API grants; a migration creating one needs explicit `GRANT`s in the same file. Not affecting us today: no migration creates a `public` table (all tables live in `core`, `workforce`, `inventory`, `operations`, `issues`, `purchases`, `booking`, `audit`, `ai`, `content`; the Data API reads `api.*` views/RPCs with grants we already set explicitly). Rule to keep: never create a table in `public`; if one is ever needed, add its `GRANT`s in the same migration. | C | Supabase email 2026-09 | T-TOUCH (any migration adding a table) | Lead | OPEN |
