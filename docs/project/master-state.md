...
| **WP4** | **Purchasing v2** | *Originally bounded direction (this row, set 2026-09-01):* supplier records, item↔supplier mapping, pack/unit/lead-time, draft→approval flow, ordered/expected/partially-received/received/variance/closed states, inventory-recount linkage. **Not** invoices/payments/accounting/supplier-APIs/WMS/autonomous ordering. | **CLOSED 2026-09-15, NARROWER THAN THIS ROW'S ORIGINAL BOUNDED DIRECTION — flag for Founder review.** What actually shipped (PR #524, migration `0120`): extends the existing Purchases append-only log (0089) with an `ordered`/`received` lifecycle (`ordered_quantity` informational-only, `received` writes Inventory exclusively via the pre-existing `api.record_inventory_stock_count` — no second source of truth), a read-only purchase-history tab, partial/over-receiving, and a duplicate-submit guard. This session's own implementation prompt explicitly named "suppliers CRM" and "approval chains" as **non-goals** — the opposite of this row's original direction, which explicitly wanted a Supplier entity, item↔supplier mapping, and a draft→approval flow. None of those three were built; there are also no `expected`/`variance`/`closed` states (only `pending`/`bought`/`ordered`/`received`), and no inventory-recount linkage beyond Receive's own canonical write. **Not verified whether this narrowing was a deliberate Founder scope-down for WP4 or an in-session instruction that unintentionally diverged from the standing roadmap — read `docs/ai/current-task.md`'s 2026-09-15 pointer and confirm with the Founder before treating Supplier/approval-flow work as either "done" or "still pending WP4."** |
