...
| Last reconciled | 2026-09-16 — **Cafe v2.2 WP4 Purchasing v2 (Ordered/Received) CLOSED, scope reconciled** (Founder confirmed 2026-09-16 that omitting a Supplier entity/item↔supplier mapping/draft→approval flow from WP4 was a deliberate product decision — those are now DEFERRED / FUTURE PURCHASING ENHANCEMENTS, not incomplete WP4 scope; see §7's WP4 row). PR #524 merged, `dev` HEAD `87966d8`; migration `0120` applied to Cloud DEV; extends the existing Purchases append-only log (0089) with an `ordered`/`received` lifecycle, `received` writing Inventory exclusively via the pre-existing `api.record_inventory_stock_count` — no second source of truth; a lost-update race found by independent review was fixed pre-apply via `pg_advisory_xact_lock`; live Preview Browser QA PASS with real numbers (23→83 pcs Order→Receive cycle, partial receiving, over-receive, duplicate-submit, JA/EN, 1440/768/375/320, Weekly Review + Inventory regression clean)). Prev: 2026-09-14 Cafe v2.2 WP3 Owner Weekly Review CLOSED (PR #518/#519/#520/#521, `dev` HEAD `95f45ee`); 2026-09-12 Cafe v2.2 WP2 Issues & Handover CLOSED (PR #517, `d7c435d`); 2026-09-11 ORUWA Design System v1 Foundation + Operations pilot DONE (PR #516, `79b211b`); 2026-09-10 Cafe v2.2 WP1 Operations CLOSED (ACCEPTED WITH EXPLICIT MVP LIMITATIONS; G1 fixed via migration `0116`, applied to Cloud DEV); 2026-09-03 Operations Cloud DEV module-ON smoke; 2026-09-01 canonical Cafe v2.2 WP sequence + acceptance model + `cto-context.md`. Next: WP5 Recipe Intelligence Lite, or a bounded quality-sweep (own Founder prompt, not yet started) |
...
- Operations WP1 is **CLOSED** (2026-09-10) — Manager/Staff UI + Cafe HACCP
  ...
  **DONE** (2026-09-11, PR #516). **WP2 Issues & Handover is CLOSED**
  ...
  migration `0120` applied to Cloud DEV — see §7 step 12 below and
  `docs/ai/current-task.md`'s 2026-09-15 pointer for full detail). Next:
  WP5 Recipe Intelligence Lite, or a bounded quality-sweep, own Founder
