...
| Last reconciled | 2026-09-18 — **Mission 8 Bounded Quality Sweep CLOSED** (PR #529 merged, `dev` HEAD `95bff6d`, non-RED — no migration touched; full Phase A audit zero P0/P1; a live-measured Recipes duplicate-request performance bug fixed (5→2 requests, ~3.5s→~1.5s); 6 bounded findings fixed incl. a live-reproduced focus-restore gap in the shared `Modal.tsx`; independent review caught and fixed 2 real regressions in the first-draft fixes before merge; full detail `docs/ai/SESSION_HANDOFF_2026-09-18.md`. **Cafe v2.2 itself remains NOT CLOSED** — next is Demo Readiness / Full Integrated Acceptance, own Founder prompt required, several Browser QA gaps (tablet/320px/EN/real-Staff-session/other-surface performance) explicitly still open, not assumed clean). Prev: 2026-09-17 **Cafe v2.2 WP5 Recipe Intelligence Lite CLOSED** (PR #525/#526, migration `0121` applied to Cloud DEV — Recipe↔Inventory ingredient mapping, controlled-unit cost estimation, allergen tri-state; see §7's WP5 row); 2026-09-16 Cafe v2.2 WP4 Purchasing v2 (Ordered/Received) CLOSED, scope reconciled (PR #524, migration `0120` applied to Cloud DEV); 2026-09-14 Cafe v2.2 WP3 Owner Weekly Review CLOSED (PR #518/#519/#520/#521, `dev` HEAD `95f45ee`); 2026-09-12 Cafe v2.2 WP2 Issues & Handover CLOSED (PR #517, `d7c435d`); 2026-09-11 ORUWA Design System v1 Foundation + Operations pilot DONE (PR #516, `79b211b`); 2026-09-10 Cafe v2.2 WP1 Operations CLOSED (ACCEPTED WITH EXPLICIT MVP LIMITATIONS; G1 fixed via migration `0116`, applied to Cloud DEV); 2026-09-03 Operations Cloud DEV module-ON smoke; 2026-09-01 canonical Cafe v2.2 WP sequence + acceptance model + `cto-context.md`. Next: Demo Readiness / Full Integrated Cafe v2.2 Acceptance, own Founder prompt, not yet started |
...
| **WP3** | **Owner Weekly Review** | Deliberately bounded management workflow: *what happened → what needs attention → what repeats → what action is required*. **Not** a generic dashboard / "Control Center". | **CLOSED 2026-09-14.** Permission-gated (`core.weekly_review.view`, owner/admin/manager only) read-only Manager popup over already-enabled modules — no new `module_code`/`tenant_modules` row. One `SECURITY INVOKER` RPC `api.weekly_review_summary` (migration `0119`) aggregating Workforce/Operations/Issues/Purchasing for one Monday-Sunday week, relying entirely on underlying RLS. Prev/Next week nav (capped at current in-progress week), drill-down into existing module popups, quiet-week empty state, module-off→`null` vs real-zero distinction. pgTAP `0060` 26/26, zero new suite failures. Live Preview Browser QA PASS: cross-module trace proven exact for Operations/Issues/Purchasing, week-boundary correctness proven live, negative Staff-permission check PASS, focus-trap PASS; one real mobile-truncation bug found and fixed (PR #520). Independent review PASS, zero P0/P1/P2. PR #518 merged (Founder, RED path); PR #519 (rollback-comment wording only) still open awaiting Founder merge. |
| **WP4** | **Purchasing v2** | *Canonical scope (reconciled 2026-09-16 by explicit Founder decision — supersedes this row's original 2026-09-01 draft direction below):* Inventory-derived purchase need, Manager review, `pending -> ordered -> received` lifecycle, ordered quantity, actual received quantity, partial receiving, over-receive per the implemented contract, canonical Inventory update through the existing Inventory mechanism, purchase history, tenant/location isolation, JA/EN, responsive operational workflow. **Deferred / future Purchasing enhancements (explicitly NOT part of Cafe v2.2 WP4, do not block WP4 CLOSED):** supplier entity/management, item↔supplier mapping, preferred supplier, supplier catalog/contacts/contracts, draft→approval workflow, multi-level approvals, automatic supplier ordering, invoices, payments, accounting, OCR, procurement analytics. *(Original 2026-09-01 draft direction, superseded: supplier records, item↔supplier mapping, pack/unit/lead-time, draft→approval flow, ordered/expected/partially-received/received/variance/closed states, inventory-recount linkage.)* | **CLOSED 2026-09-15, scope reconciled 2026-09-16.** Shipped (PR #524, migration `0120`): extends the existing Purchases append-only log (0089) with an `ordered`/`received` lifecycle (`ordered_quantity` informational-only, `received` writes Inventory exclusively via the pre-existing `api.record_inventory_stock_count` — no second source of truth), a read-only purchase-history tab, partial/over-receiving, and a duplicate-submit guard. The Founder confirmed (2026-09-16) that omitting the Supplier entity, item↔supplier mapping, and draft→approval flow from this row's original 2026-09-01 draft direction was a deliberate product decision, not an oversight — those items are reclassified above as deferred future enhancements, not incomplete WP4 scope. Full detail: `docs/ai/current-task.md`'s 2026-09-15 pointer (as reconciled 2026-09-16). |
| **WP5** | **Recipe Intelligence Lite** | Recipe↔Inventory ingredients/BOM, controlled units, allergens, **estimated operational cost** with price precedence `confirmed receiving price → manual default price → unknown`; missing price **never** shows as `0`; estimates never presented as accounting-exact. Operational cost guidance, not accounting. | **CLOSED 2026-09-17.** Shipped (PR #525/#526, migration `0121`): optional Recipe ingredient → Inventory item mapping (quantity + controlled unit), Manager-maintained allergen tri-state + reference price on Inventory items, deterministic estimated-cost calculation (`kg↔g`/`L↔mL` conversion only, never a guessed number for `pcs` or other pairs). `estimated_cost` is `NULL` — never `0` — for any unmapped/unpriced/unit-incompatible ingredient. No supplier/invoice/margin/nutrition features, no automatic Inventory consumption from Recipe config. pgTAP `0062` 44/44. Independent review PASS, zero P0/P1. Live Preview Browser QA PASS with real cost calculation, missing-price/unit-incompatible acceptance, all three allergen states, Manager/Staff visibility split, JA/EN, 1440/768/375/320. Full detail: `docs/ai/current-task.md`'s 2026-09-17 pointer. |
...
| Operations | **WP1 CLOSED 2026-09-10** — backend `0099`–`0116`, Manager/Staff UI, Cafe HACCP presets; enabled for the `oruwa-cafe` reference tenant on Cloud DEV only | §7 |
...
- Operations WP1 is **CLOSED** (2026-09-10) — Manager/Staff UI + Cafe HACCP
  ...
  **DONE** (2026-09-11, PR #516). **WP2 Issues & Handover is CLOSED**
  (2026-09-12, PR #517, migrations `0117`/`0118` applied to Cloud DEV,
  `oruwa-cafe` enabled). **WP3 Owner Weekly Review is now CLOSED**
  (2026-09-14, PR #518, migration `0119` applied to Cloud DEV). **WP4
  Purchasing v2 (Ordered/Received) is now CLOSED** (2026-09-15, PR #524,
  migration `0120` applied to Cloud DEV). **WP5 Recipe Intelligence Lite is
  now CLOSED** (2026-09-17, PR #525/#526, migration `0121` applied to Cloud
...
5. **Operations Manager/Staff UI** — **DONE** (2026-09-05, 7 PRs, live-QA'd).
6. **Cafe HACCP presets** — **DONE** (2026-09-08, PR #511/#512, applied to
   `oruwa-cafe` on Cloud DEV, live-QA'd).
7. **WP1 bounded acceptance gate** — **DONE, WP1 CLOSED 2026-09-10**
...
10. **WP3 Owner Weekly Review** → 11. **WP3 bounded acceptance** — CLOSED
   2026-09-14 (PR #518-#522).
12. **WP4 Purchasing v2** — CLOSED 2026-09-15 (PR #524, migration `0120`
   applied to Cloud DEV; see `docs/ai/current-task.md`'s 2026-09-15
...
### ORUWA Product Quality Foundation (Founder decision 2026-09-09)

A separate **platform-level** bounded phase — not a Cafe feature — that runs
**after WP1 CLOSE** and **before active WP2–WP5 implementation**. Goal: a
single ORUWA design system so WP2–WP5 and every later product are built on
one consistent, accessible, maintainable UI foundation instead of the
current ad-hoc `lib/ui/theme.ts` + per-page inline styles.

- **Bounded v1 with an explicit Definition of Done.** Not "redesign
  everything" — a first version that is complete enough to build WP2 on.
- **Structure:** `ARTIFACT` (the design system itself — tokens already
  landed as Phase 0 / `@line-os/tokens`, PR #514; then primitives,
  components, patterns), `STANDARDS` (a11y, responsive, component states,
  error recovery, performance — written norms), `AUDITS` (the evidence
  each area was checked).
- **Do NOT pre-decide the technology.** The first stage is a combined
  technical + UX audit comparing three strategies — (a) evolve the current
  hand-rolled layer, (b) a thin ORUWA component layer, (c) a
  Radix-primitives-based system — and recommends one. No commitment to
  Radix / current / bespoke before that audit.
- **Operations is the first pilot** — migrate the existing Operations
  ...
  pixel-for-pixel: evidence-based UX improvements are allowed. **No
  business-logic / DB / RPC / RLS change** without a separate Founder gate.
- **No extra acceptance gates.** Product Quality Foundation folds into the
  existing sequence; the only Cafe v2.2 runtime acceptance remains the
  per-WP gates + the single Phase-4 Integrated Acceptance. Before that
  final acceptance: a full JA/EN Copy Audit, a Founder-facing Russian
