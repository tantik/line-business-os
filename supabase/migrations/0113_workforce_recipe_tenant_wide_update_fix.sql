-- ============================================================================
-- 0113  Fix api.upsert_workforce_recipe so a Manager can edit/publish a
--       TENANT-WIDE (location_id IS NULL) recipe
-- ----------------------------------------------------------------------------
-- SAME BUG CLASS `main`'s historical 0060 fixed — which never reached `dev`
-- (main/dev diverged 2026-08-16; see docs/ai/
-- PLATFORM_FOUNDATION_RECONCILIATION_HANDOFF_2026-08-29.md). This is NOT a
-- restore of 0060: `main`'s 0060 predates `dev`'s 0081 (which widened the
-- allowed `p_status` set to include 'archived'). Applying 0060 verbatim would
-- REVERT that. This migration takes the CURRENT `dev` function body (0081) and
-- applies only the one-line fix, in both language branches.
--
-- PROBLEM (reproduced on current `dev`, 2026-08-29):
--   The UPDATE branch matched the target row with
--       where r.tenant_id = p_tenant_id
--         and r.location_id = p_location_id      -- <-- bug
--         and r.id = p_recipe_id
--   `p_location_id` is always the caller's own resolved (non-null) location.
--   A tenant-wide recipe has `location_id IS NULL`, and `NULL = <uuid>` is
--   NULL (not true), so the UPDATE always hit zero rows, `v_recipe_id` stayed
--   NULL, and the RPC raised `recipe_not_found` — surfacing to the Manager as
--   "Something went wrong" on every attempt to edit or (re-)publish a
--   tenant-wide recipe. Location-scoped recipes were unaffected.
--
-- FIX: match a tenant-wide row too, mirroring the branch `wf_recipes_update`'s
--   own RLS predicate (0022) + `core.has_permission_in_tenant` already use
--   for this table:
--       and (r.location_id = p_location_id or r.location_id is null)
--
-- SECURITY: widens NO privilege. This RPC is SECURITY INVOKER;
--   `wf_recipes_update`'s RLS `using`/`with check` is the authorization
--   boundary and ALREADY permits an UPDATE to a `location_id IS NULL` row for
--   a holder of `workforce.recipe.manage`/`.publish` anywhere in the tenant
--   (`core.has_permission_in_tenant`). This only stops the RPC's own WHERE
--   clause from being stricter than RLS for that case. A row scoped to a
--   DIFFERENT location than the caller's own remains rejected (still
--   `r.location_id = p_location_id` false, still not `is null`).
--
-- Signature unchanged from 0081 -> `create or replace` in place; grants
-- restated for an explicit record (idempotent).
--
-- DUAL-TARGET: `create or replace` converges on both — fresh local (0081 ran
-- first) and Cloud dev (0081 was pushed).
-- ============================================================================
...
-- ============================================================================
-- Rollback: restore 0081's body (the two UPDATE WHERE clauses lose the
--           `or r.location_id is null` disjunct).
-- ============================================================================
